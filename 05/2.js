...
function isDiagonal(line) {
	let a = Math.abs(line[1][0] - line[0][0]);
	let b = Math.abs(line[1][1] - line[0][1]);
	if (a > b) {
		const temp = a;
		a = b;
		b = temp;
...
function runHVDLine(grid, line) {
	if (isDiagonal(line)) {
		const y1 = Math.min(line[0][1], line[1][1]);
		const y2 = Math.max(line[0][1], line[1][1]);
		const x1 = y1 === line[0][1] ? line[0][0] : line[1][0];
		const x2 = y1 === line[0][1] ? line[1][0] : line[0][0];
		const dx = x2 > x1 ? 1 : -1;
		let x = x1;
		for (let y = y1; y <= y2; y++) {
			grid[y][x]++;
