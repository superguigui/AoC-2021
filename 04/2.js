...
function run(answers, grids) {
	...
	for (let i = 0; i < answers.length; i++) {
		...
		for (let j = 0; j < grids.length; j++) {
			const grid = grids[j];
			if (!grid.hasWon) {
				const hasWon = grid.tick(value);
				if (hasWon) {
					if (losingGrids.length > 1) {
						losingGrids.splice(losingGrids.indexOf(grid), 1);
					} else if (losingGrids.length === 1) {
						winningNumber = value;
					}
				}
			}

			if (!grid.hasWon && grid.tick(value) && losingGrids.length > 1) {
				losingGrids.splice(losingGrids.indexOf(grid), 1);
			}
		}
	}
	const loser = losingGrids[0];
	const sum = loser.sumUnmarked();
	console.log({ secondStar: sum * winningNumber });
	return null;
}
...
class Grid {
	...
	tick(value) {
		...
		this.hasWon = this.checkRows() || this.checkColumns();
		return this.hasWon;
	}
